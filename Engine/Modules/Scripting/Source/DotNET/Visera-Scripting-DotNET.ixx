...
export module Visera.Scripting.DotNET;
#define VISERA_MODULE_NAME "Scripting.DotNET"
...
namespace Visera
{
    struct VISERA_SCRIPTING_API HostFXR
    {
        ...
        InitializeForScriptingConfig           {nullptr};
    ...
    export class VISERA_SCRIPTING_API FDotNETComponent
    {
    public:
    ...
    public:
        ...
        FDotNETComponent(const FPath& I_DotNETScriptingRoot,
                         const FPath& I_DLLPath,
                         const FPath& I_ScriptingConfigPath);
    };

    export class VISERA_SCRIPTING_API FDotNETApplication
    {
    public:
        [[nodiscard]] Bool
    ...
    public:
        ...
        FDotNETApplication(const FPath&    I_DotNETScriptingRoot,
                           TArray<FPath>&& I_Args); // Commandline App
        ~FDotNETApplication()
        {
            if (HostFXR::Close(Context) != HostFXR::Success)
            { LOG_ERROR("Failed to finalize App (path:{})!", ScriptingConfigPath); }
        }
    };

    export class VISERA_SCRIPTING_API FDotNETScripting
    {
    public:
        [[nodiscard]] inline const FPath&
        GetRoot() const { return Root; }
        [[nodiscard]] inline TSharedPtr<FDotNETComponent>
        CreateComponent(const FPath& I_DLLPath, const FPath& I_ScriptingConfigPath) const;
    ...
    public:
        FDotNETScripting()
        {
            ...
            HostFXR::InitializeForScriptingConfig = reinterpret_cast<hostfxr_initialize_for_runtime_config_fn>
                (HostFXR->LoadFunction("hostfxr_initialize_for_runtime_config"));
            HostFXR::GetScriptingDelegate = reinterpret_cast<hostfxr_get_runtime_delegate_fn>
    ...
    TSharedPtr<FDotNETComponent> FDotNETScripting::
    CreateComponent(const FPath& I_DLLPath,
                    const FPath& I_ScriptingConfigPath) const
    {
        LOG_DEBUG("Creating a component (path:{}, config:{}).",
                  I_DLLPath, I_ScriptingConfigPath);
        return MakeShared<FDotNETComponent>(Root, I_DLLPath, I_ScriptingConfigPath);
    }

    TSharedPtr<FDotNETApplication> FDotNETScripting::
    CreateCommandLineApp(TArray<FPath>&& I_Args) const
    {
        if (I_Args.empty())
      ...
      ScriptingConfigPath (I_ScriptingConfigPath)
    {
            ...
            &Context) != HostFXR::Success || !Context)
        {
            LOG_ERROR("Failed to initialize Component (path:{}, config:{})!",
                      DLLPath, ScriptingConfigPath);
            return;
        }

        if (HostFXR::GetScriptingDelegate(
            Context,
    ...
    : Args(I_Args)
    {
        ...
        ScriptingConfigPath = Args[0];
        LOG_DEBUG("Initializing App (path:{}).", ScriptingConfigPath);
