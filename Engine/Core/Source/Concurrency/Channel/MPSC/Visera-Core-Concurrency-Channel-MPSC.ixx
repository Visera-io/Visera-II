...
#include <coroutine>
export module Visera.Core.Concurrency.Channel.MPSC;
...
{
    /**
     * Multi Producer / Single Consumer channel: event-driven Receive with non-blocking Send.
     * Wraps TMPSCQueue + FEvent. Misuse (multiple consumers) is UB.
     * - Send: non-blocking; enqueues then signals. Safe from multiple producer threads. Signals both blocking Receive (via FEvent) and ReceiveAsync (via WaitingHandle); both are signaled every time.
     ...
     */
    template<typename T>
    class VISERA_CORE_API TMPSCChannel final
    {
    public:
        using ElementType = T;
    ...
    private:
        ...
        TMPSCQueue<ElementType>              Queue;
        FEvent                               ItemAvailable;
        TAtomic<std::coroutine_handle<>> WaitingHandle{ nullptr };

    public:
        TMPSCChannel() = default;
        TMPSCChannel(const TMPSCChannel&)            = delete;
        TMPSCChannel& operator=(const TMPSCChannel&) = delete;
        TMPSCChannel(TMPSCChannel&&)                 = delete;
        TMPSCChannel& operator=(TMPSCChannel&&)      = delete;
    };
}
