...
export namespace Visera
{
    ...
    public:
        ...
        [[nodiscard]] VISERA_FORCEINLINE explicit constexpr operator bool() const { return HasValue();}

        [[nodiscard]] constexpr OptionalType&&
        GetValue() && { VISERA_ASSERT(HasValue()); return std::move(TypedValue); }

        [[nodiscard]] constexpr OptionalType&
        GetValue() & { VISERA_ASSERT(HasValue()); return TypedValue; }

        [[nodiscard]] VISERA_FORCEINLINE constexpr const OptionalType&
        GetValue() const & { VISERA_ASSERT(HasValue()); return TypedValue; }
        ...
        [[nodiscard]] friend constexpr Bool operator==(const TOptional& I_Lhs, const TOptional& I_Rhs)
        {
            ...
            if (bLhsSet != bRhsSet) { return False; }
            if (!bLhsSet)           { return True; }
    ...
    private:
        ...
        union { OptionalType TypedValue; }; // Disable Constructor & Destructor
        VISERA_NO_UNIQUE_ADDRESS std::conditional_t<bUsingIntrusiveUnsetState, Private::FEmpty, Bool>
        bHasValue = {};
    ...
    public:
        ...
        TOptional& operator=(const TOptional& I_Other)
        {
            ...
            {
                if (I_Other.HasValue()) { Emplace(I_Other.GetValue()); }
                else { Reset(); }
            }
            return *this;
        }
        ...
        TOptional& operator=(TOptional&& I_Other) noexcept
        {
            if (std::addressof(I_Other) != this)
            {
                if (I_Other.HasValue()) { Emplace(std::move(I_Other.GetValue())); }
                else { Reset(); }
            }
            return *this;
        }
