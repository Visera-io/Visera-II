...
export namespace Visera
{
    ...
    {
        template<class H>
        concept Handle = requires(const H h, UInt32 Generation, UInt32 Index)
        {
            { h.GetIndex()            } -> std::convertible_to<UInt32>;
            { h.GetGeneration()       } -> std::convertible_to<UInt32>;
            { h.GetValue()            } -> std::convertible_to<UInt64>;
            { h.IsNull()              } -> std::convertible_to<Bool>;
            { H{Generation, Index}    } -> std::same_as<H>;
            { H::GetGenerationRange() } -> std::convertible_to<TClosedInterval<UInt32>>;
    ...
    {
    public:
        // Default: full 32-bit generation range, excluding 0 (0 is used by Null handle)
        static constexpr TClosedInterval<UInt32>
        GetGenerationRange() { return { 1U, ~0U }; }
    ...
    public:
        ...
        constexpr FHandle(UInt32 I_Generation, UInt32 I_Index)
            : Value{ (static_cast<UInt64>(I_Generation) << 32) | static_cast<UInt64>(I_Index) }
        {}
