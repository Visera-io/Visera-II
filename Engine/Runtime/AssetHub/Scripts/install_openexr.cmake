...
macro(link_openexr in_target)
    ...
    if(NOT TARGET OpenEXR)
        ...
        if(APPLE)
            foreach(_t Iex IlmThread OpenEXRCore OpenEXR)
                target_compile_options(${_t} PRIVATE $<$<CONFIG:Debug>:-g0> $<$<CONFIG:Develop>:-g0>)
            endforeach()
            if(TARGET OpenEXRUtil)
                target_compile_options(OpenEXRUtil PRIVATE $<$<CONFIG:Debug>:-g0> $<$<CONFIG:Develop>:-g0>)
            endif()
        endif()
        set_property(TARGET OpenEXR     PROPERTY CXX_STANDARD 20)
        set_property(TARGET OpenEXRCore PROPERTY CXX_STANDARD 20)
        set_target_properties(OpenEXR PROPERTIES FOLDER "Visera/AssetHub/External/OpenEXR")
    endif()

    target_link_libraries(${in_target} PRIVATE "$<BUILD_INTERFACE:OpenEXR>")
endmacro()