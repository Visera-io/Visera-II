...
namespace Visera
{
    ...
    public:
        ...
        WriteStorageBuffer(UInt32                 I_Binding,
                           FVulkanBuffer*         I_Buffer,
                           vk::DescriptorType     I_DescriptorType = vk::DescriptorType::eStorageBuffer,
                           UInt64                 I_Range = 0,
                           UInt32                 I_ArrayElement = 0);
        VISERA_NOINLINE void
    ...
    WriteStorageBuffer(UInt32                 I_Binding,
                       FVulkanBuffer*         I_Buffer,
                       vk::DescriptorType     I_DescriptorType,
                       UInt64                 I_Range,
                       UInt32                 I_ArrayElement /* = 0 */)
    {
        if (!I_Buffer)
        ...
        const UInt64 EffectiveRange = (I_Range > 0) ? I_Range : I_Buffer->GetSize();
        auto BufferInfo = vk::DescriptorBufferInfo{}
            .setBuffer (I_Buffer->GetHandle())
            .setOffset (0)
        ...
        auto WriteInfo = vk::WriteDescriptorSet{}
            ...
            .setDstBinding      (I_Binding)
            .setDstArrayElement (I_ArrayElement)
            .setDescriptorType  (I_DescriptorType)
            .setPBufferInfo     (&BufferInfo)
        ;
