...
{
    /** CreateInfo for FRHI. Presence in FEngineCreateInfo enables the RHI service. */
    struct VISERA_RUNTIME_API FRHICreateInfo
    {
        FString PreferredGPUName = "";
        UInt64  CommandListHighWaterMarkBytes = kCommandListHighWaterMarkBytes;
    };

    class VISERA_RUNTIME_API FRHI
    {
    public:
        explicit FRHI(const FRHICreateInfo& I_CreateInfo);
        ~FRHI();
    ...
    public:
        ...
        WriteDescriptorStorageBuffer(FRHIDescriptorSetID I_DS, UInt32 I_Binding,
                                     FRHIBufferID I_Buffer, UInt64 I_Range = 0);
        void
    ...
    private:
        ...
        FString        RuntimeName = "RHI";
    };

    FRHI::FRHI(const FRHICreateInfo& I_CreateInfo)
    {
        RHIThread.PreferredGPUName = I_CreateInfo.PreferredGPUName;
        RHIThread.CommandListHighWaterMark = I_CreateInfo.CommandListHighWaterMarkBytes;
        ...
        RHIThread.Driver->CreateInstance(RuntimeName, AppVersion, InstanceExtensions);
        ...
        { RuntimeName = FString(RHIThread.Driver->GetGPU().Properties.deviceName.data()); }
    }

    FRHI::~FRHI()
    {
        if (!RHIThread.Shutdown())
        { LOG_ERROR("RHI shutdown failed."); }
        RHIThread.Stop();
        if (RHIThread.Driver)
        {
    ...
    ExecuteInitialize()
    {
        ...
        bInitSuccess = True;
        LOG_DEBUG("RHI initialized (no reserved headless slot; use CreateSwapChain(nullptr) for headless).");
        InitEvent.Trigger();
    }
    ...
    ExecuteShutdown()
    {
        ...
        if (!Driver)
        {
            ...
            LOG_DEBUG("ExecuteShutdown: no driver, done.");
            return;
        }
        ...
        for (UInt8 Idx = 0; Idx < static_cast<UInt8>(SwapChains.GetSize()); ++Idx)
        {
            ...
            if (SC.Window)
            {
                LOG_TRACE("Destroy windowed SwapChain (id:{}).", Idx);
                Driver->DestroySwapChain(SC.Window);
            }
            else
            {
                LOG_TRACE("Destroy headless SwapChain (id:{}).", Idx);
            }
        }
        {
    ...
    ExecuteCreateSwapChain(FWindow* I_Window, void* I_PreCreatedSurface)
    {
        ...
        LOG_TRACE("ExecuteCreateSwapChain: windowed (id:{}, title:{}).", SCIdx, I_Window->GetTitle());
    }
    ...
    ExecuteCreateHeadlessSwapChain()
    {
        ...
        LOG_TRACE("ExecuteCreateHeadlessSwapChain: headless (id:{}, in-flight:{}).", SCIdx, SwapChains[SCIdx].InFlightFrames.GetSize());
        return SCIdx;
    }
    ...
    ExecuteDestroySwapChain(FWindow* I_Window, FEvent* I_Done)
    {
        ...
        {
            LOG_TRACE("ExecuteDestroySwapChain: windowed (id:{}).", Idx);
    ...
    ExecuteDestroySwapChain(FRHISwapChainID I_ID, FEvent* I_Done)
    {
        ...
        LOG_TRACE("ExecuteDestroySwapChain: (id:{}, headless:{}).", I_ID, SC.Window == nullptr);
        FreeSlot(I_ID);
        if (I_Done) { I_Done->Trigger(); }
    }
    ...
    ExecuteBindDescriptorSet(FRHIInFlightFrame& I_Frame, const FRHICommandView& I_Cmd)
    {
        ...
        const UInt32* DynamicOffsetPtr = Payload.DynamicOffsetCount > 0 ? &Payload.DynamicOffset : nullptr;

        if (CurrentComputePipelineLayout)
        {
            const vk::DescriptorSet DescriptorSetHandle =
                DescriptorSet->GetVulkanDescriptorSet()->GetHandle();
            const auto ComputeBindInfo = vk::BindDescriptorSetsInfo{}
                .setLayout(CurrentComputePipelineLayout->GetHandle())
                .setStageFlags(vk::ShaderStageFlagBits::eCompute)
                .setFirstSet(Payload.SetIndex)
                .setDescriptorSetCount(1)
                .setPDescriptorSets(&DescriptorSetHandle)
                .setDynamicOffsetCount(Payload.DynamicOffsetCount)
                .setPDynamicOffsets(DynamicOffsetPtr);
            I_Frame.GraphicsCalls.GetHandle().bindDescriptorSets2(ComputeBindInfo);
        }
        else
        {
            if (!I_Frame.GraphicsCalls.IsInsideRenderPass()) { return; }
            I_Frame.GraphicsCalls.BindDescriptorSet(
                Payload.SetIndex,
                DescriptorSet->GetVulkanDescriptorSet(),
                Payload.DynamicOffsetCount,
                Payload.DynamicOffset);
        }
    }
    ...
    void FRHI::
    WriteDescriptorStorageBuffer(FRHIDescriptorSetID I_DS, UInt32 I_Binding,
                                 FRHIBufferID I_Buffer, UInt64 I_Range)
    {
        auto* DS  = RHIThread.Registry->Get(I_DS.GetHandle());
        auto* Buf = RHIThread.Registry->Get(I_Buffer.GetHandle());
        ...
        const auto& Bindings = DS->GetInfo().Bindings;
        for (const auto& Binding : Bindings)
        {
            if (Binding.Binding == I_Binding &&
                Binding.Type == ERHIDescriptorType::StorageBufferDynamic)
            {
                DescriptorType = vk::DescriptorType::eStorageBufferDynamic;
                break;
            }
        }
        DS->GetVulkanDescriptorSet()->WriteStorageBuffer(I_Binding, Buf->GetVulkanBuffer(), DescriptorType, I_Range);
    }
