...
{
    /** CreateInfo for FInput. No config fields; presence in FEngineCreateInfo enables the service. */
    struct VISERA_RUNTIME_API FInputCreateInfo
    {
    };
     ...
     */
    class VISERA_RUNTIME_API FInput
    {
    public:
        explicit FInput(const FInputCreateInfo& I_CreateInfo = FInputCreateInfo{})
        {
            (void)I_CreateInfo;
            Keyboard = MakeUnique<FKeyboard>();
            Mice[DummyWindow] = MakeUnique<FMouse>();
        }
    ...
    {
        if (!I_PlatformWindow)
        { LOG_ERROR("RegisterWindow: null platform window."); return False; }
        ...
        { LOG_ERROR("RegisterWindow: failed to bind KeyboardCallback."); return False; }
        ...
        { LOG_ERROR("RegisterWindow: failed to bind MouseButtonCallback."); return False; }
        ...
        { LOG_ERROR("RegisterWindow: failed to bind CursorMoveCallback."); return False; }
        ...
        { LOG_ERROR("RegisterWindow: failed to bind ScrollCallback."); return False; }

        Mice.InsertOrAssign(I_PlatformWindow, MakeUnique<FMouse>());
        LOG_DEBUG("Registered platform window for input (Mice size {}).", Mice.GetSize());
        return True;
    }
    ...
    FInput::UnregisterWindow(IPlatformWindow* I_PlatformWindow)
    {
        ...
        LOG_DEBUG("Unregistered platform window (Mice size {}).", Mice.GetSize());
    }
    ...
    void FInput::NotifyCursorMove(IPlatformWindow* I_SourceWindow, Float I_PosX, Float I_PosY)
    {
        ...
        if (!TargetMouse)
        { 
            LOG_WARN("NotifyCursorMove: no target mouse found for window.");
    ...
    void FInput::NotifyScroll(IPlatformWindow* I_SourceWindow, Float I_OffsetX, Float I_OffsetY)
    {
        ...
        if (!TargetMouse)
        {
            LOG_WARN("NotifyScroll: no target mouse found for window.");
            TargetMouse = Mice[DummyWindow].Get();
         }
    ...
    FInput::RebuildReverseIndex()
    {
        ...
        LOG_DEBUG("Rebuilt reverse index ({} entries).", ReverseIndex.GetSize());
    }
